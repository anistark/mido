...
Badge images, the shields.io row that READMEs open with, are drawn as labels instead of image placeholders: a two-tone chip with the badge name on the left and its value on the right, linked wherever the badge links, so `]` and `Enter` follow it. The value side takes the badge's own color, read from the URL of a static badge or from the SVG of a fetched one, with dark or light text picked for contrast. The name side stays on the theme unless the URL sets `labelColor`. mido recognises the shields.io, badgen, badge.fury.io, forthebadge and deps.rs hosts, plus any URL with a `badge` segment such as GitHub's `badge.svg`. A static shields badge carries its text in the URL, so it shows in full offline. A live badge such as a crate version only knows its name until mido fetches it: start with `--remote-images` and the value is read from the badge's SVG title, so `crates.io` becomes `crates.io v0.4.0`. Nothing is downloaded without that flag.
