...
source text
  ...
  -> Document   blocks and inlines, each block with its   src/markdown/document.rs
...
- `src/render/wrap.rs` wraps styled text without splitting a style run, `syntax.rs` wraps syntect, `theme.rs` maps semantic tokens to styles, and `mermaid/` turns diagram blocks into text.
...
1. **Model it.** Add a variant to `BlockKind` in `src/markdown/document.rs`. Keep it data only: what the block contains, nothing about how it looks.
2. **Parse it.** In `src/markdown/parse.rs` the `Builder` turns pulldown-cmark events into a tree of `Node` frames. A block with children needs a `Node` variant that collects them, a `start` arm for its `Tag`, and an `end_frame` arm that pushes the finished `Block` with its byte span through `self.block`. Blocks without children, like a rule, are pushed straight from the event.
3. **Draw it.** In `src/render/layout.rs` add an arm to `Renderer::block` and a method beside `quote`, `list` and `table`. Build styled pieces, wrap them with `wrap` at `self.avail()`, and emit lines with `emit_line`. For nested content push a prefix with `push_prefix`, render the inner blocks with `self.blocks`, then `pop_prefix`. Colors come from `src/render/theme.rs`: add a semantic token there rather than using a raw color, and give it a value in the indexed and mono fallbacks.
